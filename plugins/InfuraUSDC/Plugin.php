...
class Plugin extends AbstractPlugin implements PaymentInterface
{
    public function boot(): void
    {
        $this->filter('available_payment_methods', function ($methods) {
            if ($this->getConfig('enabled', true)) {
                $methods['InfuraUSDC'] = [
                    'name'        => $this->getConfig('display_name', 'USDC (ERC20)'),
    ...
    public function form(): array
    {
        return [
            'wallet_address' => [
                ...
                'type'        => 'string',
                'required'    => true,
                'description' => '你的 ETH 钱包地址（0x 开头），与 USDT 可用同一地址'
            ],
            'infura_api_key' => [
    ...
    private function getUsdcRate(): float
    {
        // USDC 同样锚定美元，直接复用 USDT 汇率缓存
    ...
    public function pay($order): array
    {
        ...
        $usdcAmount   = round($cnyAmount / $rate, 2);
        $uniqueAmount = $usdcAmount + (rand(1, 99) / 10000);
        $amountStr    = number_format($uniqueAmount, 4, '.', '');

        // key 区分 usdc
        Redis::setex('lumina_database_usdc_order_' . $amountStr, 1800, $order['trade_no']);
        ...
        return [
            'type' => 1,
            'data' => '/pay-usdc-erc20/index.html?' . $query,
        ];
    }
