...
class MailService
{
    ...
    public static function sendEmail(array $params)
    {
        ...
        try {
            $html = view($params['template_name'], $params['template_value'])->render();
            $apiKey = env('RESEND_API_KEY');
            $fromAddress = admin_setting('email_from_address', config('mail.from.address'));
            $fromName = admin_setting('app_name', 'XBoard');
            $payload = json_encode(['from' => $fromName . ' <' . $fromAddress . '>', 'to' => [$email], 'subject' => $subject, 'html' => $html]);
            ...
            curl_close($ch);
            if ($httpCode !== 200 && $httpCode !== 201) { throw new \Exception('Resend API error: ' . $response); }
            $error = null;
        } catch (\Exception $e) {
            Log::error($e);
            $error = $e->getMessage();
